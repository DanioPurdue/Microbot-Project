...
public:
	...
	void writeForceInformation(Mat& img_display, Point matchLoc ,int probeRows, int probeCols,int probeLocX,int probeLocY, int bodyLocX, int bodyLocY)
	{
		...
		putText(img_matches, ss1.str(), Point(3, img_matches.rows / 8 * 7.1 - offset), cv::FONT_HERSHEY_TRIPLEX, 1.3, cv::Scalar(127, 127, 127));
		...
		putText(img_matches, ss2.str(), Point(3, img_matches.rows / 8 * 7.4 - offset), cv::FONT_HERSHEY_TRIPLEX, 1.3, cv::Scalar(127, 127, 127));
		
        //draw circle on the target
		circle(img_matches, targetLocation, 20.0, cv::Scalar(255, 255, 255), 1, 8);

		std::stringstream ss3;
		ss3 << "Y Component Force[microN]: " <<  abs(force2D.forceY);
		putText(img_matches, ss3.str(), Point(3, img_matches.rows / 8 * 7.7 - offset), cv::FONT_HERSHEY_TRIPLEX, 1.3, cv::Scalar(127, 127, 127));

		std::stringstream ss4;

		ss4 << "X Component Force[microN]: " <<  force2D.forceX;
		putText(img_matches, ss4.str(), Point(3, img_matches.rows / 8 * 8 - offset + 5), cv::FONT_HERSHEY_TRIPLEX, 1.3, cv::Scalar(127, 127, 127));
	...
	struct Force2D ForceCalculator(int probeLocX, int probeLocY, int bodyLocX, int bodyLocY, int framePixelLength)
	{
		...
		else
		{
			...
			if (abs(force2D.forceY) <= 2 || abs(force2D.forceY) > 16)	//this is added to eliminate the noise
			{
				force2D.forceY = 0;
			}
