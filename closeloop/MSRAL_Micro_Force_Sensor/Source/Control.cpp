...
void Control::move()
{
	...
	int angle_delta = abs(robot.angle - angle) % 360;
	if (min(angle_delta, 360 - angle_delta)> 60) {
		//rotate to angle
		rotate(angle);
		return;
	}
...
void Control::push()
{
	...
	if (maxForceCounter > 40) {
		state = state_halt;
		maxForce = robot.force;
		msg = "Maximum force applied 5 times.";
	}
	int timeDelay = 1;
	if (robot.force >= maxForce - timeDelay) {
		maxForceCounter++;
