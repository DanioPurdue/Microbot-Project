...
public:
	...
	struct Force2D ForceCalculator(int probeLocX, int probeLocY, int bodyLocX, int bodyLocY, int framePixelLength)
	{
		...
		int distanceY = sqrt(pow(overall_distance, 2) + pow(framePixelLength/2, 2));
	...
	{
		return 0.51 * deformationDistance;
	}
	...
	int forceXStack(int forceX)
	{
		...
		if (abs(averageForce - forceX) > 2 || abs(forceXHist[1] - forceX) > 3)
		return averageForce;
	...
	Point colorTrackingProbeBlue(cv::Mat imageOriginal)
	{
		...
		Mat imgThresholded;
		inRange(imgHSV, cv::Scalar(79, 68, 54), cv::Scalar(105, 217, 219), imgThresholded); //Threshold the image
		...
		imshow("Thresholded Image Body 2", imgThresholded); //show the thresholded 
		cv::waitKey(3);
		return cntr;
	}
	...
	Point colorTrackingSpring(cv::Mat imageOriginal)
	{
		...
		//Display the result
		//imshow("Thresholded Image2", imgThresholded); //show the thresholded 
		//cv::waitKey(3);
		return cntr;
