		  ...
		  while (!isStopped()) {
			  ...
			  if (callIt) { 
			    ...
			    fnumber++;
				  ...
				  //namedWindow(windowNameOutput, WINDOW_AUTOSIZE);
				  //moveWindow(windowNameOutput, 120, 100);
				  float zoom_ratio = 0.8;
				  //cv::resize(output, output, cv::Size(floor(1280*zoom_ratio), floor(1024*zoom_ratio)));
				  cv::imshow(windowNameOutput, output);
				  //std::cout << "Image Size: " << output.rows << " " << output.cols << std::endl;
				  //resizeWindow(windowNameOutput, 900, 900);
				  cv::waitKey(1);
			  }	
