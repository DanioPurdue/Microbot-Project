...
uint32_t HIDUniversal::Init(uint32_t parent, uint32_t port, uint32_t lowspeed) {
		...
		TRACE_USBHOST(printf("HIDUniversal::Init\r\n");)
        ...
        if(rcode) {
                ...
                bAddress = 0;
				TRACE_USBHOST(printf("setAddr:%d\r\n", rcode);)
                return rcode;
        }

        //delay(2); //per USB 2.0 sect.9.2.6.3

		TRACE_USBHOST(printf("Addr:%d\r\n", bAddress);)
		...
		TRACE_USBHOST(printf("HU configured\r\n");)
...
void HIDUniversal::EndpointXtract(uint32_t conf, uint32_t iface, uint32_t alt, uint32_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
        ...
        if(index) {
				...
				epInfo[bNumEP].deviceEpNum = (pep->bEndpointAddress & 0x0F);
				...
				if (pipe == 0)
				{
					TRACE_USBHOST(printf("HIDUniversal::EndpointXtract : Pipe allocation failure\r\n");)
					// Enumeration failed
					return;
				}
...
uint32_t HIDUniversal::Poll() {
        ...
        if((long)(millis() - qNextPollTime) >= 0L) {
                ...
                for(uint8_t i = 0; i < bNumIface; i++) {
                        ...
                        uint8_t rcode = pUsb->inTransfer(bAddress, epInfo[index].epAddr, (uint32_t*)&read, buf);
                        ...
                        Notify(PSTR("\r\n"), 0x80);
#endif
                        ParseHIDData(this, bHasReportId, (uint32_t)read, buf);

                        HIDReportParser *prs = GetReportParser(((bHasReportId) ? *buf : 0));
						
                        if(prs)
                                prs->Parse(this, bHasReportId, (uint32_t)read, buf);
                }
        }
        return rcode;
}
